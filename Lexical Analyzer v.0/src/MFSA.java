...
public class MFSA {
	...
	public MFSA(DFSA obj,ArrayList terminals,Vector expressions) {
	...
	private void printVector(Vector v) {
		for (int i = 0; i < v.size(); i++) {
			...
			if ( ( (Group) v.get(i)).tokenRecognized == -1) {
				...
				System.out.println("Token Regognized " +((Expression) this.expressions.get(( ( (Group) v.get(i)).
						tokenRecognized))).name);
			}
			...
			for (int i = 0; i < v.size(); i++) {
				...
				for (int j = 0; j < group.statesList.size(); j++) {
					...
					for (int k = 0; k < temp.size(); k++) {
						if (! ( (String) temp.get(k)).equals( ( (String) group.inputChar.get(k)))) {
	...
	public ArrayList getDistination(Group element, Vector v) {
		...
		for (int i = 0; i < element.inputChar.size(); i++) {
			...
			if(((Integer)element.inputChar.get(i)).intValue()==-1){
				temp.add("" + -1);
			}
			else{
	...
	//--------------------------------------------------------------------------
	public void move(String stream) {
		output = new ArrayList();
		Group startState = (Group) ( (Group) groups.get(0)).statesList.get(0);
			...
			if (index == -1) {
				...
				if (currentState.tokenRecognized != -1) {
					...
					System.out.println("< " + stream.substring(start, end) + " , " +
							((Expression) this.expressions.get(currentState.tokenRecognized)).name   + " >");
					...
					if (lastAcceptedState != null && end > start) {
						...
						System.out.println("< " + stream.substring(start, end) + " , " +
								((Expression) this.expressions.get(lastAcceptedState.tokenRecognized)).name + " >");
						lastAcceptedState =null;
					}
				}
