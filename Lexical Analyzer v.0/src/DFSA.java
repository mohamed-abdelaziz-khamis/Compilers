...
/*
 Vector of ArrayLists:
  Each ArrayList of objects of type State:
    Each State has:
       symbol: character
       nextState: integer
...
public class DFSA {
    ...
    private ArrayList LambdaClosureState(int state, Vector NDFSA) {
        ...
        for (int i = 0; i < ((ArrayList) NDFSA.get(state)).size(); i++) {
            ...
            if (((State) (((ArrayList) NDFSA.get(state)).get(i))).ch == 200) {
                Union(x,
                      LambdaClosureState(((State) (((ArrayList) NDFSA.get(state)).
    ...
    private void fillStates(ArrayList states, Vector NDFSA, ArrayList terminals) {
        State2 state; ArrayList transition;
        int i, j, k;
        for (i = 0; i < NDFSA.size(); i++) {
            ...
            for (j = 0; j < terminals.size(); j++) {
                ...
                for (k = 0; k < ((ArrayList) NDFSA.get(i)).size(); k++) {
                    ...
                    if (((State) ((ArrayList) NDFSA.get(i)).get(k)).ch ==
                        ((character) terminals.get(j)).terminal)

                        transition.add(new Integer(((State) ((ArrayList) NDFSA.get(i)).get(k)).nextState));
            }
                state.transitionsUnderTerminals.add(transition);
            }
            states.add(state);
        }
    }
    ...
    private ArrayList move(ArrayList LambdaClosure, int i, ArrayList states) {
        ArrayList x = new ArrayList();
        int Index ;  State2 s;  ArrayList transitions;  ArrayList transition;
        for (int k = 0; k < LambdaClosure.size(); k++) {
            Index = ((Integer)LambdaClosure.get(k)).intValue();

            s =(State2) states.get(Index);

            transitions= s.transitionsUnderTerminals;

            if(!transitions.isEmpty()){
                transition = (ArrayList) transitions.get(i);
                Union(x, transition);
            }
        }
        return x;
    ...
    private int findToken(ArrayList transitions, Vector NDFSA) {
        ...
        for (int i = 0; i < transitions.size(); i++)
            if (((State) ((ArrayList) (NDFSA.get(((Integer) (transitions.get(i))).
                                                intValue()))).get(0)).ch ==
                203)
                return ((State) ((ArrayList) (NDFSA.get(((Integer) (transitions.
