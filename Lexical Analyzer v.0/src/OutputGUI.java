...
public class OutputGUI extends JFrame {
	...
	ArrayList terminals;
	Vector expressions;
	public OutputGUI(MFSA mfsa,DFSA dfsa,ArrayList terminals,Vector expressions ) throws HeadlessException {
	...
	implements DocumentListener {
		public void insertUpdate(DocumentEvent e) {
			...
			try {
				...
				outputs = mfsa.output;
				SwingUtilities.invokeLater(new write(attr,doc,outputs,doc_out,expressions)) ;
	...
	protected JMenuBar createMenuBar() {
		...
		Action actionNDFSA = new AbstractAction("Show Miminum Groups"){
			public void actionPerformed(ActionEvent e){
				MFSAGUI mfsagui = new MFSAGUI(mfsa.groups);
				//JMenuItem item = new JMenuItem(actionDFSA);
			}
		};
...
class write implements Runnable{
	...
	write(MutableAttributeSet attr,DefaultStyledDocument doc,ArrayList outputs,DefaultStyledDocument doc_out,Vector expressions){
		this.attr = attr;
		this.doc = doc;
	...
	public void run() {
		...
		try {
			...
			s = "< "+doc.getText(((Out) outputs.get(i)).start , ((Out) outputs.get(i)).end - ((Out) outputs.get(i)).start)+" , "+((Expression) this.expressions.get(((Out) outputs.get(i)).token)).name +" >";
